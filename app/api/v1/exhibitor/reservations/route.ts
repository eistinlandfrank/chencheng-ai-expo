...
export async function PATCH(request: NextRequest) {
  ...
  if (!payload.reservation_id || !['confirmed', 'completed', 'cancelled'].includes(String(payload.status))) return NextResponse.json({ code: 'VALIDATION_FAILED', message: '操作内容不完整', request_id: requestId }, { status: 422 });
